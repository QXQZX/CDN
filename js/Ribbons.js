...
"object" == typeof window && (window.Ribbons = function() {
    ...
    return e.prototype = {
        ...
        init: function() {
            try {
                this._canvas = document.createElement("canvas"), this._canvas.style.display = "block", this._canvas.style.position = "fixed", this._canvas.style.margin = "0", this._canvas.style.padding = "0", this._canvas.style.border = "0", this._canvas.style.outline = "0", this._canvas.style.left = "0", this._canvas.style.top = "0", this._canvas.style.width = "100%", this._canvas.style.height = "100%", this._canvas.style["z-index"] = "0", this._canvas.id = "bgCanvas", this._onResize(), this._context = this._canvas.getContext("2d"), this._context.clearRect(0, 0, this._width, this._height), this._context.globalAlpha = this._options.colorAlpha, window.addEventListener("resize", this._onResize), window.addEventListener("scroll", this._onScroll), document.body.appendChild(this._canvas)
            } catch (t) {
        ...
        addRibbon: function() {
            ...
            for (var l = [], c = new h(a, r), _ = new h(a, r), p = null, d = Math.round(o(0, 360)), u = 0; !(i <= 0);) {
                if (i--, s = Math.round((1 * Math.random() - 0.2) * this._options.horizontalSpeed), e = Math.round((1 * Math.random() - 0.5) * (0.25 * this._height)), (p = new h).copy(_), "right" === t) {
                    if (p.add(s, e), _.x >= n) {
                        break
                    }
                } else {
                    if ("left" === t && (p.subtract(s, e), _.x <= -200)) {
                        break
                    }
                }
                l.push({
                    ...
                    alpha: 0,
                    phase: 0
                }), c.copy(_), _.copy(p), u += 4, d += this._options.colorCycleSpeed
            }
